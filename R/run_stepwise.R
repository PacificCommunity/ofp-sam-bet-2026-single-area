...
for (i in seq_len(nrow(step_table))) {
  ...
  if (isTRUE(save_raw_mfcl_inputs)) {
    ...
    if (!isTRUE(raw_mfcl_inputs_saved)) {
      ...
      # These small reports are generated by the fitted run (parest flag 246)
      # and cannot be reconstructed from model_payload.rds alone.  Native
      # jitter needs indepvar.rpt to perturb only active parameters.
      for (name in c("indepvar.rpt", "xinit.rpt")) {
        generated <- file.path(model_dir, name)
        if (file.exists(generated)) {
          file.copy(
            generated,
            file.path(raw_mfcl_inputs_dir, name),
            overwrite = TRUE,
            copy.date = TRUE
          )
        }
