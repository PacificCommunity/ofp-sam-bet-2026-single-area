...
KFLOW_TASK ?= $(call yml,y$$name,ofp-sam-bet-2026-single-area)
...
help:
	@printf '%s\n' \
	  'BET 2026 single-area shortcuts' \
	  '' \
	  ...
	  '  Refresh Kflow task definitions for the single-area model, results, and report from their kflow.yaml files.' \
	  '' \
...
local: readme
	...
	KFLOW_REPO_RUNTIME_UPDATE='$(KFLOW_REPO_RUNTIME_UPDATE)' \
...
docker: readme
	...
	docker run --rm \
	  ...
	  -e KFLOW_REPO_RUNTIME_UPDATE='$(KFLOW_REPO_RUNTIME_UPDATE)' \
